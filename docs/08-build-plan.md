...
- **Implemented (US-017):** `.github/workflows/ci.yml` runs `npm run check`/`lint`/`build` on every PR and on push to `main`. `.github/workflows/deploy.yml` builds with `BASE_PATH=/<repo-name>` (this repo isn't named `<owner>.github.io`, so it serves from a project subpath until the custom domain is wired up — see the comment in `vite.config.ts`) and deploys via the official `actions/upload-pages-artifact` + `actions/deploy-pages` flow on push to `main`.
- **Requires one manual, one-time repo setting change before the deploy workflow will succeed:** in the GitHub repo's Settings → Pages → "Build and deployment" → Source, select **"GitHub Actions"** (not "Deploy from a branch"). This can't be done from a commit/PR — it's a dashboard setting only a repo admin can change.
- **Acceptance:** Production URL serves the site correctly over HTTPS; Lighthouse scores from `00-overview.md`'s success criteria are met on the live URL, not just locally; PRs are blocked from merging if typecheck/lint/build fail. The live-URL and Lighthouse checks can only be performed after this PR is merged to `main` *and* the Pages source setting above is changed — verified locally instead by confirming the build succeeds both with and without `BASE_PATH` set (simulating the exact env the deploy workflow uses) and that every internal asset/link path resolves correctly under the subpath.
